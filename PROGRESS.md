...
| 5. Preview, chọn release, import Room, Cover Art Archive và dedupe | DONE + HOTFIX | Chờ commit mới | 2026-08-18 | Cover Art Archive đã tách OkHttpClient/ImageLoader, follow redirect, fallback release-group → release, 404 NotFound, Coil cache và lưu local `filesDir/covers/{albumId}.jpg`; import progress là modal. Chờ push CI và xác nhận thiết bị. |
| 6. Credits MusicBrainz, bảng Credit và tra cứu theo người | DONE + HOTFIX | Chờ commit mới | 2026-08-18 | Credits đã gộp người/vai trò/nhạc cụ, có số đếm nhóm, tối đa 3 nhóm mở và footer nguồn MusicBrainz; chờ push CI và xác nhận thiết bị. |
...
Sandbox không có emulator hoặc `adb` (`adb-not-installed`), nên không thể thực hiện logcat thực tế, kiểm thử tay, cài APK, xác minh dialog không bị bottom navigation che, hoặc chụp bốn ảnh bắt buộc. Không đoán thay cho bằng chứng. A–C hiện đã compile sạch; full `assembleDebug`, `testDebugUnitTest` và `lintDebug` PASS ở local. Chưa có ảnh chụp giả và chưa báo hoàn thành sản phẩm. Cần push CI xanh, build APK gửi người dùng, sau đó người dùng xác minh trên thiết bị thật trước khi đánh dấu hotfix hoàn tất và trước khi chuyển Phase 8.
