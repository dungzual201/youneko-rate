...
| 4. Network, throttle, cache, MusicBrainz search và release lookup | DONE + HOTFIX | `aeeb80a` + hotfix commit | 2026-08-18 | Hotfix online search bổ sung DTO `@SerialName`/defaults theo response thật, escape + UTF-8 encode query Lucene, không cache count=0/empty/error, dọn cache rỗng cũ, Paging dùng top-level count và empty-state hiển thị keyword. Local full assemble/unit test/lint PASS; CI [Android Build #32122723283](https://github.com/dungzual201/youneko-rate/actions/runs/32122723283) PASS với assemble debug APK, debug unit tests, MainActivity Robolectric launch regression và artifact upload. |
| 5. Preview, chọn release, import Room, Cover Art Archive và dedupe | DONE | `ba6b9b8` | 2026-08-18 | Đã triển khai release-group → release preview, release selector, import Room theo batch, Cover Art Archive front-500/front-250/cat fallback, local cover copy, MBID/normalized dedupe và non-destructive merge. Local compile/unit test/lint PASS; CI [Android Build #32102023444](https://github.com/dungzual201/youneko-rate/actions/runs/32102023444) PASS với assemble debug APK, debug unit tests và upload artifacts. |
| 6. Credits MusicBrainz, bảng Credit và tra cứu theo người | DONE | Phase 6 commit | 2026-08-18 | Credits lazy-load theo album/track, grouped credits, relation fallback recording/work, cache TTL 30 ngày, progress/hủy, empty-state đóng góp MusicBrainz và dedupe Unicode/role. Full `assembleDebug`, `testDebugUnitTest` và `lintDebug` PASS; CI [Android Build #32108772138](https://github.com/dungzual201/youneko-rate/actions/runs/32108772138) PASS với assemble debug APK, debug unit tests, MainActivity Robolectric launch regression và artifact upload. |
...
Sandbox không có emulator hoặc `adb` (`adb-not-installed`), nên không thể thực hiện bước logcat request thực tế, kiểm thử tay 8 trường hợp, cài APK, hoặc chụp screenshot thiết bị. Vì vậy phân loại A/B/C/D từ logcat chưa thể xác nhận trực tiếp trong sandbox; không đoán thay cho bằng chứng. Hotfix được kiểm tra bằng response thật đã lưu, fixture/parser/mapper/query/cache tests, full `assembleDebug`, `testDebugUnitTest` và `lintDebug` PASS. Không tạo screenshot giả; người dùng phải xác minh online search trên thiết bị thật sau CI.
