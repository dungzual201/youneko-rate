...
| 6. Credits MusicBrainz, bảng Credit và tra cứu theo người | DONE + HOTFIX | IN-PROGRESS SHA | 2026-08-18 | Đính chính parser theo fixture thật AMORTAGE: đọc release-level + recording-level + work-level relations, lưu begin/end, gộp semantic role/attribute và hiển thị album→track buckets. Full local verification PASS; chờ commit/CI mới. |
| 7. Provider phụ và quản lý nguồn/token/cache | IN-PROGRESS | Chưa push | 2026-08-18 | Discogs API/service đã triển khai: mặc định tắt, token DataStore, cache 30 ngày, client/rate limiter riêng 25 req/phút, merge credit/source chip và clear cache UI. Last.fm toggle/key có UI; Last.fm/Deezer/ListenBrainz service còn TODO. |
| 8. Audio Quality Checker phần 1: decode, FFT, spectrogram, technical info | IN-PROGRESS | Chưa push | 2026-08-18 | Đã triển khai MediaExtractor/MediaCodec decode-only, ba đoạn 30 giây, FFT 4096 Hann/hop 2048, cutoff/rolloff/crest factor/true peak/clipping, heuristic verdict, spectrum JSON, foreground WorkManager, file picker và Analyze Canvas. Cần test matrix thiết bị thật; chưa có spectrogram PNG export đầy đủ. |
| 9. Audio Quality Checker phần 2: heuristics, verdict, lưu, so sánh, badge | IN-PROGRESS | Chưa push | 2026-08-18 | Verdict/reasons/confidence và lưu AudioAnalysis đã được đưa vào Phase8 v1; so sánh/badge lịch sử và batch folder còn TODO. |
...
Sandbox không có emulator hoặc `adb` (`adb-not-installed`), nên không thể thực hiện logcat thực tế, kiểm thử tay, cài APK, xác minh dialog không bị bottom navigation che, decoder MediaCodec trên codec matrix, hoặc chụp ảnh. Không đoán thay cho bằng chứng. Sau parser/Discogs/Phase8 v1, local `assembleDebug`, `testDebugUnitTest` và `lintDebug` đều PASS ngày 2026-08-18; `AudioAnalysisTest` và `PlaybackCapabilityGuardTest` PASS. Chưa push/CI cho lượt thay đổi này, chưa có APK mới và chưa có ảnh chụp thật; không báo hoàn thành sản phẩm cho đến khi CI xanh và người dùng xác minh trên thiết bị.
