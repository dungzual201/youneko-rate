...
private fun openBackupDatabase(context: Context, directory: File): YounekoDatabase =
    Room.databaseBuilder(context, YounekoDatabase::class.java, File(directory, BACKUP_DATABASE_ENTRY).absolutePath)
        .addMigrations(
            ...
            YounekoDatabase.MIGRATION_16_17, YounekoDatabase.MIGRATION_17_18, YounekoDatabase.MIGRATION_18_19,
        ).build()
        ...
        snapshot.analyses.forEach { value ->
            database.audioAnalysisDao().upsert(
                AudioAnalysisEntity(
                    ...
                    formatVerdict = value.formatVerdict, transcodeVerdict = value.transcodeVerdict, rawHeaderHex = value.rawHeaderHex,
                ),
            )
        }
