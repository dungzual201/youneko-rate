...
class MusicHoardersApiTest {
    ...
    fun postNdjsonEmitsCoverAndStatusPerLine() = runTest {
        MockWebServer().use { server ->
            ...
            assertTrue(request?.getHeader("User-Agent")?.contains("Android; personal library use") == true)
            assertEquals(32, request?.getHeader("X-Session")?.length)
            assertEquals("application/x-ndjson", request?.getHeader("Accept"))
            assertEquals("", request?.getHeader("X-Page-Referrer"))
            assertEquals("", request?.getHeader("X-Page-Query"))
            assertEquals("https://covers.musichoarders.xyz/", request?.getHeader("Referer"))
            assertEquals("https://covers.musichoarders.xyz", request?.getHeader("Origin"))
